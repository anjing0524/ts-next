            ...
            if global_idx < items.len() {
                match mode {
                    RenderMode::Heatmap => {
                        ...
                        if volume > 0.0 { // Only draw tooltip if there's volume in the hovered tick
...
impl OverlayRenderer {
    pub fn new() -> Self {
        let dash_array = js_sys::Array::of2(&OVERLAY_CROSSHAIR_DASH_VALUE.into(), &OVERLAY_CROSSHAIR_DASH_VALUE.into());
    ...
    pub fn handle_mouse_move(
        ...
        if distance >= OVERLAY_MOUSE_MOVE_REDRAW_THRESHOLD || self.hover_candle_index != prev_hover_candle_index || !prev_mouse_in_chart {
    ...
    fn draw_crosshair(&self, ctx: &OffscreenCanvasRenderingContext2d, layout: &ChartLayout) {
        ...
        ctx.line_to(layout.chart_area_x + layout.chart_area_width, mouse_y_constrained);
        ctx.stroke();
        let mouse_x_constrained = self.mouse_x.max(layout.chart_area_x).min(layout.chart_area_x + layout.chart_area_width);
        ctx.begin_path();
        ctx.move_to(mouse_x_constrained, layout.header_height);
        ctx.line_to(mouse_x_constrained, layout.header_height + layout.chart_area_height);
        ctx.stroke();
    ...
    fn draw_axis_labels(
        ...
        let adjusted_y_label_y = y_label_y.max(layout.header_height).min(layout.header_height + layout.chart_area_height - y_label_height);
        ...
        let _ = ctx.fill_text(&y_label_text, y_label_x + y_label_width - Y_AXIS_LABEL_X_OFFSET, adjusted_y_label_y + y_label_height / 2.0);
        ...
        let adjusted_x_label_x = x_label_x.max(layout.chart_area_x).min(layout.chart_area_x + layout.chart_area_width - x_label_width);
        ...
        if let Some(hover_idx) = hover_candle_index {
            if hover_idx < items.len() {
                ...
                ctx.set_text_baseline("middle");
                let _ = ctx.fill_text(&format!("{} {}", date_str, time_str), adjusted_x_label_x + x_label_width / 2.0, x_label_y + x_label_height / 2.0);
            }
        }
    }

    fn draw_tooltip(&self, ctx: &OffscreenCanvasRenderingContext2d, layout: &ChartLayout, item: KlineItem, mode: RenderMode) {
        let timestamp = item.timestamp() as i64;
        ...
        if tooltip_x + tooltip_width > chart_right_edge { tooltip_x = self.mouse_x - mouse_offset - tooltip_width; }
        if tooltip_x < layout.chart_area_x { tooltip_x = layout.chart_area_x; }
        if tooltip_y < layout.header_height { tooltip_y = self.mouse_y + mouse_offset; }
        if tooltip_y + tooltip_height > chart_bottom_edge { tooltip_y = chart_bottom_edge - tooltip_height; }
        ...
        ctx.quadratic_curve_to(tooltip_x + tooltip_width, tooltip_y, tooltip_x + tooltip_width, tooltip_y + corner_radius);
        ctx.line_to(tooltip_x + tooltip_width, tooltip_y + tooltip_height - corner_radius);
        ctx.quadratic_curve_to(tooltip_x + tooltip_width, tooltip_y + tooltip_height, tooltip_x + tooltip_width - corner_radius, tooltip_y + tooltip_height);
        ctx.line_to(tooltip_x + corner_radius, tooltip_y + tooltip_height);
        ctx.quadratic_curve_to(tooltip_x, tooltip_y + tooltip_height, tooltip_x, tooltip_y + tooltip_height - corner_radius);
    ...
    pub fn check_switch_button_click(&self, x: f64, y: f64, layout: &ChartLayout) -> Option<RenderMode> { 
        let button_width = layout.switch_btn_width * 2.0; 
        let button_height = layout.switch_btn_height;
        let button_x = (layout.canvas_width - button_width) / 2.0; 
        let button_y = layout.padding;
    ...
    fn draw_heatmap_tooltip(&self, ctx: &OffscreenCanvasRenderingContext2d, layout: &ChartLayout, timestamp: i64, price: f64, volume: f64) {
        let datetime_str = time::format_timestamp(timestamp, FORMAT_STR_DATETIME_YMDHM);
        let tooltip_width = OVERLAY_TOOLTIP_WIDTH;
        let line_height = OVERLAY_TOOLTIP_LINE_HEIGHT;
        let padding = OVERLAY_TOOLTIP_PADDING;
    ...
    pub fn redraw(
        ...
        self.render_component(canvas_manager, &canvas_manager.layout.borrow(), data_manager, mode);
    }
