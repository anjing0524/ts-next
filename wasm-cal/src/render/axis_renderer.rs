...
impl AxisRenderer {
    ...
    fn draw_alternating_background(
        ...
        for i in 0..num_y_bands {
            ...
            let color = if i % 2 == 0 { ChartColors::BACKGROUND } else { ChartColors::GRID };
            ctx.set_fill_style_str(color);
            ctx.fill_rect(layout.chart_area_x, band_y, layout.main_chart_width, band_height);
        }
        ...
        for i in 0..vol_num_y_bands {
            ...
            let color = if i % 2 == 0 { ChartColors::BACKGROUND } else { ChartColors::GRID };
            ctx.set_fill_style_str(color);
            ctx.fill_rect(layout.chart_area_x, band_y, layout.main_chart_width, vol_band_height);
        }
    }
    ...
    fn sample_price_ticks(
        ...
        if tick <= 0.0 { return Vec::new(); } // Added guard from previous step
        ...
        if tick_count > max_labels && max_labels > 0 {
            ...
            if step == 0 { return tick_vec; } // Added guard from previous step
            for (i, &v) in tick_vec.iter().enumerate() {
                if i % step == 0 { sampled_ticks.push(v); }
            }
        } else {
    ...
    fn draw_y_axis_labels(&self, params: YAxisLabelParams<'_>) {
        for &v in params.values {
            let y = match &params.axis_type {
                AxisType::Price { min_low, max_high } => params.layout.map_price_to_y(v, *min_low, *max_high),
                AxisType::Volume { max_volume } => params.layout.map_volume_to_y(v, *max_volume) - if v == 0.0 { AXIS_Y_LABEL_PRICE_ZERO_OFFSET } else { 0.0 },
            };
    ...
    fn draw_price_y_axis(
        ...
        if price_range <= 0.0 || tick <= 0.0 { return; }
        
        let sampled_ticks = self.sample_price_ticks(min_low, max_high, tick, layout.price_chart_height);
        let label_points: Vec<(f64, f64)> = sampled_ticks.iter().map(|&price| (price, layout.map_price_to_y(price, min_low, max_high))).collect();
    ...
    fn draw_volume_y_axis(
        ...
        ctx.fill_rect(0.0, layout.volume_chart_y, layout.y_axis_width, layout.volume_chart_height);
        ctx.set_stroke_style_str(ChartColors::BORDER);
        ctx.set_line_width(CL_DEFAULT_CROSSHAIR_WIDTH);
        ctx.begin_path();
        ctx.move_to(layout.y_axis_width, layout.volume_chart_y);
        ctx.line_to(layout.y_axis_width, layout.volume_chart_y + layout.volume_chart_height);
        ctx.stroke();
        ctx.set_fill_style_str(ChartColors::AXIS_TEXT);
        ctx.set_font(ChartFont::AXIS);
        ctx.set_text_align("right");
        ...
        let values: Vec<f64> = (0..=num_y_labels).map(|i| max_volume * i as f64 / num_y_labels as f64).filter(|v| v.is_finite()).collect();
    ...
    fn draw_header(
        ...
        ctx.fill_rect(legend_x, legend_y - AXIS_LEGEND_RECT_Y_OFFSET, AXIS_LEGEND_RECT_SIZE, AXIS_LEGEND_RECT_SIZE);
        ctx.set_fill_style_str(ChartColors::TEXT);
        ...
        ctx.fill_rect(legend_x + AXIS_LEGEND_TEXT_X_OFFSET_2, legend_y - AXIS_LEGEND_RECT_Y_OFFSET, AXIS_LEGEND_RECT_SIZE, AXIS_LEGEND_RECT_SIZE);
        ctx.set_fill_style_str(ChartColors::TEXT);
    ...
    fn draw_x_axis(
        ...
        if max_labels == 0 { return; } // Added guard
        ...
        for i in (0..visible_count).step_by(candle_interval) {
            ...
            let _ = ctx.fill_text(&time_str, x, time_label_y_start + TIME_AXIS_SECOND_LINE_Y_OFFSET);
        }
    }
}
