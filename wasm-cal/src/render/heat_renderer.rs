...
impl HeatRenderer {
    pub fn new() -> Self {
        ...
        for i in 0..HEATMAP_COLOR_CACHE_SIZE {
            // Use constant for loop bound
        ...
        for (rel_idx, bins) in all_bins.iter().enumerate() {
            ...
            for (bin_idx, &volume) in bins.iter().enumerate() {
                ...
                ctx.fill_rect(
                    x_left,
                    rect_y,
                    x_width - HEATMAP_RECT_BORDER_ADJUST,
                    rect_height - HEATMAP_RECT_BORDER_ADJUST,
                );
    ...
    fn get_cached_color(&self, norm: f64) -> String {
        ...
        if index < self.color_cache.len() {
            ...
            if index >= HEATMAP_COLOR_CACHE_SIZE && HEATMAP_COLOR_CACHE_SIZE > 0 {
                ...
                // Covers HEATMAP_COLOR_CACHE_SIZE == 0 and other fallbacks
