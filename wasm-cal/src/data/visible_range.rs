...
impl VisibleRange {
    ...
    pub fn get_screen_coordinates(&self, layout: &ChartLayout) -> (f64, f64) {
        ...
        let visible_end_x =
            layout.chart_area_x + (self.start + self.count) as f64 * nav_candle_width;
    ...
    pub fn zoom_with_relative_position(
        ...
        let new_start = ((visible_center_idx - (new_visible_count as f64 * relative_position))
            .round() as isize)
            .max(0)
            .min((self.total_len.saturating_sub(new_visible_count)) as isize)
            as usize;
    ...
    pub fn calculate_data_ranges(
        ...
        let (min_low, max_high, max_volume) = (self.start..self.end).fold(
            ...
            |(min_l, max_h, max_v), idx| {
                // Renamed to avoid conflict
                if idx >= items.len() {
                    return (min_l, max_h, max_v);
                }
                let item = items.get(idx);
                (
                    min_l.min(item.low()),
                    max_h.max(item.high()),
                    max_v.max(item.b_vol() + item.s_vol()),
                )
            },
        );

        let (final_min_low, final_max_high) =
            if (max_high - min_low).abs() < MIN_PRICE_DIFF_THRESHOLD {
                (
                    min_low - VISIBLE_RANGE_DEFAULT_PRICE_BUFFER / 2.0,
                    max_high + VISIBLE_RANGE_DEFAULT_PRICE_BUFFER / 2.0,
                )
            } else {
