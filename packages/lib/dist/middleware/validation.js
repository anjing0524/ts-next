...
async function validateRequest(request, options = {}) {
    ...
    if (options.rateLimit) {
        ...
        const rateLimitResult = utils_1.RateLimitUtils.applyRateLimit(request, config);
 ...
 * @returns 验证结果
 */
function validatePKCE(codeChallenge, codeChallengeMethod, required = false) {
    // 如果不是必需的且没有提供，则跳过验证
    ...
    // 验证代码挑战格式
    if (codeChallenge && !auth_1.PKCEUtils.validateCodeChallenge(codeChallenge)) {
        return {
            valid: false,
            response: server_1.NextResponse.json({
                error: 'invalid_request',
