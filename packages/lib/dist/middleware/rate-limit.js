...
exports.withOAuthRateLimit = withOAuthRateLimit;
exports.withIPRateLimit = withIPRateLimit;
exports.withUserRateLimit = withUserRateLimit;
 ...
 * @returns 速率限制键
 */
function generateRateLimitKey(request, options) {
...
function getClientIP(request) {
    ...
    if (xForwardedFor) {
        ...
        return ((_a = xForwardedFor.split(',')[0]) === null || _a === void 0 ? void 0 : _a.trim()) || '127.0.0.1';
    }
    if (xRealIP)
        return xRealIP;
    if (cfConnectingIP)
...
function withRateLimit(handler, options = {}) {
    ...
    const rateLimitOptions = Object.assign(Object.assign({}, DEFAULT_RATE_LIMIT_OPTIONS), options);
        ...
        try {
            ...
            if (!rateLimitResult.allowed) {
                ...
                if (rateLimitOptions.includeHeaders) {
                    addRateLimitHeaders(errorResponse, rateLimitOptions.maxRequests, rateLimitResult.remaining, rateLimitResult.resetTime);
                }
            ...
            // 添加速率限制头部 (Add rate limit headers)
            if (rateLimitOptions.includeHeaders) {
                addRateLimitHeaders(response, rateLimitOptions.maxRequests, rateLimitResult.remaining, rateLimitResult.resetTime);
            }
            return response;
        }
        catch (error) {
 ...
 * @returns 包装后的中间件函数
 */
function withOAuthRateLimit(handler, maxRequests = 60, windowMs = 60 * 1000 // 1分钟 (1 minute)
) {
 ...
 * @returns 包装后的中间件函数
 */
function withIPRateLimit(handler, maxRequests = 100, windowMs = 15 * 60 * 1000 // 15分钟 (15 minutes)
) {
    return withRateLimit(handler, {
        maxRequests,
        windowMs,
 ...
 * @returns 包装后的中间件函数
 */
function withUserRateLimit(handler, maxRequests = 200, windowMs = 15 * 60 * 1000 // 15分钟 (15 minutes)
) {
    return withRateLimit(handler, {
        maxRequests,
        windowMs,
