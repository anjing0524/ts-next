...
exports.SALT_ROUNDS = exports.PasswordComplexitySchema = void 0;
...
function generateSecurePassword(length = 12) {
    ...
    password += LOWERCASE_CHARACTERS[crypto_1.default.randomInt(LOWERCASE_CHARACTERS.length)]; // 添加一个随机小写字母
    password += NUMBER_CHARACTERS[crypto_1.default.randomInt(NUMBER_CHARACTERS.length)]; // 添加一个随机数字
    ...
    try {
        exports.PasswordComplexitySchema.parse(password); // 使用 Zod Schema 验证密码
        ...
        // 确保至少一个小写字母和一个数字
        fallbackPassword += charSets[0][crypto_1.default.randomInt(charSets[0].length)];
        fallbackPassword += charSets[2][crypto_1.default.randomInt(charSets[2].length)];
        // 填充剩余长度
        for (let i = 2; i < length; i++) {
            fallbackPassword += allChars[crypto_1.default.randomInt(allChars.length)];
        }
        // 再次打乱
 ...
 */
async function checkPasswordHistory(userId, // 用户ID
newPasswordRaw, // 新的明文密码
historyLimit = 5 // 检查最近多少条历史记录，默认为5
) {
    // 从数据库中查询指定用户的密码历史记录。
    // 按创建时间降序排序，并只取最近的 `historyLimit` 条记录。
    const passwordHistory = await client_1.prisma.passwordHistory.findMany({
 ...
 */
exports.SALT_ROUNDS = 10; // 标准化盐轮数，通常10-12是一个好的平衡点。
