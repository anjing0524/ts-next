 ...
 */
function handleError(error, req, requestId) {
    const currentRequestId = requestId || (0, uuid_1.v4)(); // 确保有一个请求ID (Ensure there is a request ID)
    ...
    if (error instanceof errors_1.BaseError) {
        ...
        if (apiResponse.error) {
            apiResponse.error.details = Object.assign(Object.assign({}, apiResponse.error.details), { requestId: currentRequestId });
        }
        return server_1.NextResponse.json(apiResponse, { status: error.status });
    }
    ...
    return server_1.NextResponse.json(genericErrorResponse, { status: 500 });
}
/**
...
function withErrorHandling(handler) {
    return async (req, params) => {
        const requestId = (0, uuid_1.v4)(); // 为每个请求生成唯一的ID (Generate a unique ID for each request)
 ...
 * ```
 */
async function safeExecute(fn, failureMessage = 'Operation failed', ErrorType = errors_1.ValidationError // 默认使用具体的错误类型
) {
    try {
        ...
        const requestId = (0, uuid_1.v4)(); // 为此错误事件生成ID (Generate ID for this error event)
 ...
 */
function errorResponse(message, status, errorCode) {
    return server_1.NextResponse.json({
        success: false,
