 ...
 * 
 * PKCE (RFC 7636) 是一种用于增强 OAuth 2.0 公共客户端安全性的机制
 * PKCE (RFC 7636) is a mechanism to enhance OAuth 2.0 public client security
 * 主要用于防止授权码拦截攻击
...
export class PKCEUtils {
   ...
   */
  static isSupportedChallengeMethod(method: string): boolean {
   ...
   */
  static validatePKCEParams(params: PKCEParams): PKCEValidationResult {
    ...
    if (codeChallengeMethod && !this.isSupportedChallengeMethod(codeChallengeMethod)) {
      return {
        isValid: false,
    ...
    if (codeVerifier && !this.validateCodeVerifier(codeVerifier)) {
      return {
        isValid: false,
    ...
    if (codeVerifier && codeChallenge && codeChallengeMethod) {
      const isMatch = this.verifyCodeChallenge(codeVerifier, codeChallenge, codeChallengeMethod);
   ...
   */
  static generatePKCEPair(): { codeVerifier: string; codeChallenge: string; codeChallengeMethod: string } {
