/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const __wbg_klineprocess_free: (a: number, b: number) => void;
export const klineprocess_new: (a: any, b: number, c: number) => [number, number, number];
export const klineprocess_set_canvases: (a: number, b: any, c: any, d: any) => [number, number];
export const klineprocess_draw_all: (a: number) => [number, number];
export const klineprocess_handle_mouse_move: (a: number, b: number, c: number) => void;
export const klineprocess_get_cursor_style: (a: number, b: number, c: number) => [number, number];
export const klineprocess_handle_mouse_leave: (a: number) => void;
export const klineprocess_handle_wheel: (a: number, b: number, c: number, d: number) => void;
export const klineprocess_handle_mouse_down: (a: number, b: number, c: number) => void;
export const klineprocess_handle_mouse_up: (a: number, b: number, c: number) => void;
export const klineprocess_handle_mouse_drag: (a: number, b: number, c: number) => void;
export const klineprocess_set_render_mode: (a: number, b: number, c: number) => [number, number];
export const klineprocess_handle_canvas_resize: (a: number, b: number, c: number) => void;
export const klineprocess_update_config: (a: number, b: any) => [number, number];
export const klineprocess_get_config: (a: number) => [number, number, number];
export const klineprocess_get_theme: (a: number) => [number, number, number];
export const __wbg_performancemonitor_free: (a: number, b: number) => void;
export const performancemonitor_new: () => number;
export const performancemonitor_enable: (a: number) => void;
export const performancemonitor_disable: (a: number) => void;
export const performancemonitor_start_frame: (a: number) => void;
export const performancemonitor_end_frame: (a: number) => void;
export const performancemonitor_record_draw_call: (a: number) => void;
export const performancemonitor_set_candles_rendered: (a: number, b: number) => void;
export const performancemonitor_set_indicators_rendered: (a: number, b: number) => void;
export const performancemonitor_frame_start: (a: number) => void;
export const performancemonitor_get_current_fps: (a: number) => number;
export const performancemonitor_get_render_time: (a: number) => number;
export const performancemonitor_get_memory_usage_mb: (a: number) => number;
export const performancemonitor_get_memory_percentage: (a: number) => number;
export const performancemonitor_get_metrics_json: (a: number) => [number, number];
export const performancemonitor_get_metrics_wasm: (a: number) => [number, number, number];
export const performancemonitor_get_snapshot_wasm: (a: number) => [number, number, number];
export const performancemonitor_init_monitor: (a: number) => void;
export const performancemonitor_get_performance_stats: (a: number) => [number, number, number, number];
export const start: () => void;
export const __wbindgen_malloc: (a: number, b: number) => number;
export const __wbindgen_realloc: (a: number, b: number, c: number, d: number) => number;
export const __wbindgen_exn_store: (a: number) => void;
export const __externref_table_alloc: () => number;
export const __wbindgen_export_4: WebAssembly.Table;
export const __wbindgen_free: (a: number, b: number, c: number) => void;
export const __externref_table_dealloc: (a: number) => void;
export const __wbindgen_start: () => void;
