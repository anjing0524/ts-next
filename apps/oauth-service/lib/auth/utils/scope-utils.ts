/**
 * OAuth 2.0 Scope 工具类
 * OAuth 2.0 Scope Utility Class
 * 
 * 用于处理OAuth 2.0权限范围的验证、解析和格式化
 * Used for OAuth 2.0 scope validation, parsing and formatting
 * 
 * @author OAuth团队
 * @since 1.0.0
 */
...
export class ScopeUtils {
  ...
  static validateScopes(
    scopes: string[],
    clientOrAllowedScopes: OAuthClient | string[]
  ):
    | Promise<{ valid: boolean; invalidScopes: string[]; error_description?: string }>
    | { valid: boolean; invalidScopes: string[]; error_description?: string } {
    if (scopes.length === 0) {
    ...
    const client = clientOrAllowedScopes as OAuthClient;
    ...
    return (async () => {
      ...
      const invalidAgainstDb = scopes.filter((scope) => !validDbScopeNames.includes(scope));

      return {
        valid: invalidAgainstDb.length === 0,
        invalidScopes: invalidAgainstDb,
        error_description: invalidAgainstDb.length > 0 ? `Requested scope(s) not found in system: ${invalidAgainstDb.join(', ')}` : undefined,
      };
    })();
  }
   ...
   * @param userScopes - 用户拥有的权限范围数组。 (Array of scopes the user has.)
   * @param requiredScope - 需要检查的权限范围。 (The scope to check for.)
   * @returns 如果用户拥有该权限范围则返回 true，否则返回 false。 (Returns true if the user has the scope, false otherwise.)
   */
  ...
  /**
   * 检查用户是否拥有指定权限范围中的任意一个。
   * (Checks if the user has any of the specified scopes.)
   * @param userScopes - 用户拥有的权限范围数组。 (Array of scopes the user has.)
   * @param requiredScopes - 需要检查的权限范围数组。 (Array of scopes to check for.)
   * @returns 如果用户拥有任意一个权限范围则返回 true，否则返回 false。 (Returns true if the user has any of the scopes, false otherwise.)
   */
   ...
   * @param userScopes - 用户拥有的权限范围数组。 (Array of scopes the user has.)
   * @param requiredScopes - 需要检查的权限范围数组。 (Array of scopes to check for.)
   * @returns 如果用户拥有所有权限范围则返回 true，否则返回 false。 (Returns true if the user has all scopes, false otherwise.)
   */
