...
const ApiDetailsSchema = z.object({
  ...
  endpoint: z
    .string()
    .startsWith('/', "API端点路径必须以'/'开头 (Endpoint path must start with '/')"), // 端点路径，必须以 '/' 开头。
...
const CreatePermissionSchema = z
  .object({
    ...
    name: z
      ...
      .regex(
        /^[a-zA-Z0-9_:-]+$/,
        '权限名称格式无效，只能包含字母、数字、下划线、冒号和连字符 (Invalid permission name format. Allowed: letters, numbers, underscore, colon, hyphen)'
      ),
    // 权限显示名称 (用于UI展示)
    displayName: z
      .string()
      .min(1, '显示名称不能为空 (Display name cannot be empty)')
      .max(150, '显示名称不能超过150字符'),
    // 权限描述 (可选)
  ...
  .superRefine((data, ctx) => {
    // `superRefine` 用于进行依赖于多个字段的复杂验证。
    // 根据选择的权限类型 (type)，验证相应的 details 对象是否已提供。
    if (data.type === PermissionType.API && !data.apiDetails) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "API类型的权限必须提供apiDetails对象 (For API type permission, 'apiDetails' object must be provided)",
        path: ['apiDetails'],
      });
    }
    if (data.type === PermissionType.MENU && !data.menuDetails) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "MENU类型的权限必须提供menuDetails对象 (For MENU type permission, 'menuDetails' object must be provided)",
        path: ['menuDetails'],
      });
    }
    if (data.type === PermissionType.DATA && !data.dataDetails) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "DATA类型的权限必须提供dataDetails对象 (For DATA type permission, 'dataDetails' object must be provided)",
        path: ['dataDetails'],
      });
    }
  });
  ...
  try {
    ...
    const createdPermission = await prisma.$transaction(async (tx) => {
      ...
      if (type === PermissionType.API && apiDetails) {
        ...
        if (menuExists === 0) {
          ...
          throw new Error(
            `关联的菜单ID "${menuDetails.menuId}" 无效或不存在 (The associated Menu ID "${menuDetails.menuId}" is invalid or does not exist)`
          );
        }
        // 创建 MenuPermission 记录。
        await tx.menuPermission.create({
          data: { permissionId: newPermission.id, menuId: menuDetails.menuId },
        });
      } else if (type === PermissionType.DATA && dataDetails) {
        // 创建 DataPermission 记录。
        await tx.dataPermission.create({
          data: { permissionId: newPermission.id, ...dataDetails },
        });
      }
      // 从事务中返回新创建的权限基础记录。
    ...
    await AuthorizationUtils.logAuditEvent({
      ...
      metadata: {
        ...
        apiDetails,
        menuDetails,
        dataDetails, // Log the input details
      },
    });
