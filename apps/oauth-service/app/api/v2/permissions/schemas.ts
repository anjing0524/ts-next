...
export const permissionResponseSchema = z.object({
  ...
  name: z
    .string()
    .describe('权限的唯一编码名称 (Unique coded name for the permission, e.g., users:create)'),
  displayName: z.string().describe('权限的显示名称 (Display name for the permission)'),
  description: z
    .string()
    .nullable()
    .optional()
    .describe('权限的详细描述 (Detailed description of the permission)'),
  resource: z
    .string()
    .describe("权限关联的资源 (Resource associated with the permission, e.g., 'user', 'article')"),
  action: z
    .string()
    .describe(
      "权限允许的操作 (Action allowed by the permission, e.g., 'create', 'read', 'update', 'delete')"
    ),
...
export const permissionListQuerySchema = z.object({
  ...
  sortBy: z
    ...
    .default('name')
    .optional()
    .describe('排序字段 (Sort by field, e.g., name, resource, createdAt)'),
  sortOrder: z
    .enum(['asc', 'desc'])
    .default('asc')
    .optional()
