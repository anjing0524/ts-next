...
const ApiDetailsUpdateSchema = z.object({
  ...
  endpoint: z
    .string()
    .startsWith('/', "API端点路径必须以'/'开头 (Endpoint path must start with '/')")
    .optional(), // 端点路径 (可选)。
...
const UpdatePermissionSchema = z.object({
  displayName: z
    ...
    .max(150, '显示名称长度不能超过150字符')
    .optional(),
  description: z.string().max(255, '描述信息长度不能超过255字符').optional().nullable(),
  // `name` 和 `type` 字段在此处被省略，因为它们通常不应通过更新操作修改。
  resource: z
    .string()
    .min(1, '资源标识不能为空 (Resource identifier cannot be empty)')
    .max(200, '资源标识长度不能超过200字符')
    .optional(),
  action: z
    .string()
    .min(1, '操作不能为空 (Action cannot be empty)')
    .max(50, '操作名称长度不能超过50字符')
    .optional(),
  ...
  try {
    ...
    if (!permission) {
      ...
      return NextResponse.json(
        { message: '权限定义未找到 (Permission definition not found)' },
        { status: 404 }
      );
    }
    ...
    console.error(
      `获取权限 ${permissionId} 详情失败 (Failed to fetch permission details for ID ${permissionId}):`,
      error
    );
    await AuthorizationUtils.logAuditEvent({
      userId: performingAdminId,
    ...
    return NextResponse.json(
      {
        message:
          '获取权限定义详情时发生错误 (An error occurred while retrieving permission definition details)',
      },
      { status: 500 }
    );
  }
...
async function updatePermissionHandler(
  ...
  try {
    ...
    if (!existingPermission) {
      ...
      return NextResponse.json(
        { message: '权限定义未找到，无法更新 (Permission definition not found, cannot update)' },
        { status: 404 }
      );
    }
    ...
    if ((updateData as any).name || (updateData as any).type) {
      ...
      return NextResponse.json(
        {
          message:
            '权限的 "name" (名称) 和 "type" (类型) 字段不可修改 (The "name" and "type" fields of a permission cannot be changed)',
        },
        { status: 400 }
      );
    }
    ...
    const updatedPermissionResult = await prisma.$transaction(async (tx) => {
      ...
      if (updateData.displayName !== undefined)
        basePermissionUpdateData.displayName = updateData.displayName;
      if (updateData.description !== undefined)
        basePermissionUpdateData.description = updateData.description; // 允许设为 null
      if (updateData.resource !== undefined)
        basePermissionUpdateData.resource = updateData.resource;
      if (updateData.action !== undefined) basePermissionUpdateData.action = updateData.action;
      if (updateData.isActive !== undefined)
        basePermissionUpdateData.isActive = updateData.isActive;
      ...
      if (existingPermission.type === PermissionType.API && updateData.apiDetails) {
        ...
        if (updateData.menuDetails.menuId) {
          ...
          if (menuExists === 0) {
            throw new Error(
              `关联的菜单ID "${updateData.menuDetails.menuId}" 无效或不存在 (The associated Menu ID "${updateData.menuDetails.menuId}" is invalid or does not exist)`
            );
          }
        }
    ...
    console.error(
      `更新权限 ${permissionId} 失败 (Failed to update permission ID ${permissionId}):`,
      error
    );
...
async function deletePermissionHandler(
  ...
  try {
    ...
    if (rolesWithPermissionCount > 0) {
      ...
      return NextResponse.json(
        {
          message: `此权限仍被 ${rolesWithPermissionCount} 个角色使用，无法删除 (This permission is still in use by ${rolesWithPermissionCount} roles and cannot be deleted)`,
        },
        { status: 409 }
      ); // 409 Conflict
    }
    ...
    console.error(
      `删除权限 ${permissionId} 失败 (Failed to delete permission ID ${permissionId}):`,
      error
    );
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2003') {
      errorMessage =
        'Cannot delete permission as it might still be referenced by other system records';
