// 文件路径: app/api/v2/.well-known/openid-configuration/route.ts
// 描述: OAuth 2.0 Authorization Server Metadata (RFC 8414)
//       OpenID Connect Discovery 1.0 incorporates and extends this.

import { NextRequest, NextResponse } from 'next/server';
import { withErrorHandler } from '@repo/lib/utils/error-handler'; // 基本错误处理
 ...
 *   get:
 *     summary: OpenID Connect Provider Configuration Information & OAuth 2.0 Authorization Server Metadata
 *     description: |
 *       Provides configuration information about the OpenID Provider and OAuth 2.0 authorization server.
 *       Clients use this metadata to configure their interaction with the server.
 *       This endpoint consolidates OIDC Discovery 1.0 and RFC 8414.
 *     tags:
 *       - OAuth V2 (Discovery)
 *     produces:
 *       - application/json
 *     responses:
 *       '200':
 *         description: Successfully returns the OIDC provider and OAuth server metadata.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                 introspection_endpoint_auth_methods_supported: { type: array, items: { type: string }, description: "自省端点支持的资源服务器认证方法 (Supported Resource Server Authentication Methods for Introspection Endpoint)", example: ["client_secret_basic", "client_secret_post"] }
