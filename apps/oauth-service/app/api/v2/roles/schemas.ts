...
export const roleCreateSchema = z.object({
  name: z
    .string()
    .min(1, '角色名称不能为空 (Role name is required)')
    .max(100, '角色名称长度不能超过100个字符 (Role name must be at most 100 characters)'),
  displayName: z
    .string()
    .max(100, '显示名称长度不能超过100个字符 (Display name must be at most 100 characters)')
    .optional()
    .nullable(),
  description: z
    .string()
    .max(500, '描述信息长度不能超过500个字符 (Description must be at most 500 characters)')
    .optional()
    .nullable(),
...
export const roleUpdateSchema = z
  ...
  .strict(
    '请求体包含不允许更新的字段 (Request body includes fields not allowed for update via PUT)'
  );
...
export const rolePatchSchema = roleUpdateSchema
  .partial()
  .strict(
    '请求体包含不允许更新的字段 (Request body includes fields not allowed for update via PATCH)'
  );
...
export const rolePermissionAssignmentSchema = z.object({
  permissionIds: z
    .array(
      z
        .string()
        .cuid({ message: '每个权限ID必须是有效的CUID (Each permission ID must be a valid CUID)' }),
      {
        required_error:
          "权限ID列表permissionIds是必需的 (Permission IDs array 'permissionIds' is required)",
        invalid_type_error:
          'permissionIds必须是字符串ID的数组 (permissionIds must be an array of string IDs)',
      }
    )
    .min(
      0,
