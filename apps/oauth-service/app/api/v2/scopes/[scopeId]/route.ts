...
async function getScopeHandler(
  ...
  if (!z.string().cuid().safeParse(scopeId).success) {
    return NextResponse.json(
      { message: '无效的Scope ID格式 (Invalid Scope ID format)' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { message: '获取Scope详情失败 (Error fetching scope details)' },
      { status: 500 }
    );
  }
}
...
async function updateScopeHandler(
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2025') {
        return NextResponse.json(
          { message: '权限范围未找到，无法更新 (Scope not found, cannot update)' },
          { status: 404 }
        );
      }
    }
...
async function deleteScopeHandler(
  ...
  try {
    ...
    if (!scopeToDelete) {
      return NextResponse.json(
        { message: '权限范围未找到，无法删除 (Scope not found, cannot delete)' },
        { status: 404 }
      );
    }
    ...
    if (clientsUsingScope > 0) {
      return NextResponse.json(
        {
          message: `Scope "${scopeToDelete.name}" is still in use by ${clientsUsingScope} client(s) and cannot be deleted.`,
        },
        { status: 409 }
      );
    }
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2025') {
        return NextResponse.json(
          { message: '权限范围未找到，无法删除 (Scope not found, P2025)' },
          { status: 404 }
        );
      }
    }
