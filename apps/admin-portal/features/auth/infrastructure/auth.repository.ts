...
export class AuthRepository implements IAuthRepository {
  async login(credentials: { username: string; password: string }): Promise<{ accessToken: string; refreshToken: string; expiresIn: number; refreshExpiresIn: number }> {
  ...
  async refreshAccessToken(refreshToken: string): Promise<{ accessToken: string; refreshToken: string; expiresIn: number; refreshExpiresIn: number }> {
    ...
    throw new Error('Method not implemented: refreshAccessToken. It is handled internally by apiFetch.');
  }
