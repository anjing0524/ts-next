...
import { Checkbox } from '@/components/ui/checkbox'; // Assuming Checkbox can be used for Grant Types / Response Types
...
export function ClientFormDialog({
  ...
  const [selectedGrantTypes, setSelectedGrantTypes] = useState<Set<string>>(new Set(formData.grantTypes));
  const [selectedResponseTypes, setSelectedResponseTypes] = useState<Set<string>>(new Set(formData.responseTypes));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.clientName.trim() || (!client && !formData.clientId.trim())) {
        toast({variant: "destructive", title: "错误", description: "客户端名称和客户端ID不能为空。"});
        return;
    }
    if (stringToArray(formData.redirectUris).length === 0 && selectedGrantTypes.has('authorization_code')) {
        toast({variant: "destructive", title: "错误", description: "使用授权码流程时，至少需要一个重定向URI。"});
        return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg md:max-w-xl">
        <DialogHeader>
          ...
          {client && <DialogDescription>修改客户端 “{client.clientName}” (ID: {client.clientId}) 的配置。</DialogDescription>}
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="clientId" name="clientId" value={formData.clientId} onChange={handleInputChange} className="col-span-3" disabled={!!client} required />
            </div>
            {/* Redirect URIs */}
            <div className="grid grid-cols-4 items-start gap-4">
              <Label htmlFor="redirectUris" className="text-right pt-1">重定向URI</Label>
              <Textarea id="redirectUris" name="redirectUris" value={formData.redirectUris} onChange={handleInputChange} className="col-span-3" placeholder="每行一个URI" rows={3}/>
            </div>
            {/* Grant Types */}
            <div className="grid grid-cols-4 items-start gap-4">
              ...
              <div className="col-span-3 space-y-2">
                {ALL_AVAILABLE_GRANT_TYPES.map(gt => (
                  <div key={gt.id} className="flex items-center space-x-2">
                    <Checkbox id={`gt-${gt.id}`} checked={selectedGrantTypes.has(gt.id)} onCheckedChange={() => handleCheckboxGroupChange('grantTypes', gt.id, setSelectedGrantTypes)} />
                    <Label htmlFor={`gt-${gt.id}`} className="font-normal">{gt.label}</Label>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="col-span-3 space-y-2">
                {ALL_AVAILABLE_RESPONSE_TYPES.map(rt => (
                  <div key={rt.id} className="flex items-center space-x-2">
                    <Checkbox id={`rt-${rt.id}`} checked={selectedResponseTypes.has(rt.id)} onCheckedChange={() => handleCheckboxGroupChange('responseTypes', rt.id, setSelectedResponseTypes)} />
                    <Label htmlFor={`rt-${rt.id}`} className="font-normal">{rt.label}</Label>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="scope" name="scope" value={formData.scope} onChange={handleInputChange} className="col-span-3" placeholder="例如: openid profile email offline_access" />
            </div>
            {/* JWKS URI */}
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="jwksUri" className="text-right">JWKS URI</Label>
              <Input id="jwksUri" name="jwksUri" value={formData.jwksUri} onChange={handleInputChange} className="col-span-3" placeholder="(可选, 用于 private_key_jwt)" />
            </div>
            {/* Logo URI */}
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="logoUri" className="text-right">Logo URI</Label>
              <Input id="logoUri" name="logoUri" value={formData.logoUri} onChange={handleInputChange} className="col-span-3" placeholder="(可选)" />
            </div>
          </div>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={isLoading}>取消</Button>
            <Button type="submit" disabled={isLoading}>
              {isLoading ? (client ? '保存中...' : '创建中...') : (client ? '保存更改' : '创建客户端')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
