...
import { requirePermission } from '@/lib/auth/middleware'; // 引入 requirePermission 高阶函数 (Import requirePermission Higher-Order Function)
 // 引入 AuthenticatedRequest 类型定义 (Import AuthenticatedRequest type definition)
 ...
 */
async function createUserHandlerInternal(req: NextRequest): Promise<NextResponse> {
 ...
 *         description: 禁止访问（权限不足）。 (Forbidden (insufficient permissions).)
 */
async function listUsersHandlerInternal(req: NextRequest): Promise<NextResponse> {
