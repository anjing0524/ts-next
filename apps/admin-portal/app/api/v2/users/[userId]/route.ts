...
import { requirePermission } from '@/lib/auth/middleware'; // 引入权限控制中间件 (Import permission control middleware)
 // 引入认证请求类型 (Import AuthenticatedRequest type)
 ...
 */
async function getUserByIdHandlerInternal(req: NextRequest, context: RouteContext): Promise<NextResponse> {
 ...
 */
async function updateUserHandlerInternal(req: NextRequest, context: RouteContext): Promise<NextResponse> {
  const { params } = context;
  const targetUserId = params.userId;
  const performingAdmin = req.user;
  const ipAddress = req.ip || req.headers?.get('x-forwarded-for');
 ...
 *         description: 邮箱已存在冲突。 (Email already exists (conflict).)
 */
async function patchUserHandlerInternal(req: NextRequest, context: RouteContext): Promise<NextResponse> {
  const { params } = context;
  const targetUserId = params.userId;
  const performingAdmin = req.user;
  const ipAddress = req.ip || req.headers?.get('x-forwarded-for');
 ...
 */
async function deleteUserHandlerInternal(req: NextRequest, context: RouteContext): Promise<NextResponse> {
  const { params } = context;
  const targetUserId = params.userId;
  const performingAdmin = req.user;
  const ipAddress = req.ip || req.headers?.get('x-forwarded-for');
