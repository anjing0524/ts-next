 ...
 */
async function getPasswordPoliciesHandler(request: NextRequest) {
  try {
 ...
 *       type: object
 *       properties: { minLength: {type: "integer"}, requireUppercase: {type: "boolean"} }
 *     PasswordHistoryPolicy: # Define this
 *       type: object
 *       properties: { historyCount: {type: "integer"} }
 *     PasswordExpirationPolicy: # Define this
 *       type: object
 *       properties: { maxAgeDays: {type: "integer"} }
 */
async function updatePasswordPoliciesHandler(request: NextRequest) {
  let payload;
  try {
