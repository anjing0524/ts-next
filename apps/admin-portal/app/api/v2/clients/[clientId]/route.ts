...
async function getClientByIdHandler(req: NextRequest, context: RouteContext) {
  const resolvedParams = await context.params;
  const targetDbId = resolvedParams.clientId;
...
async function updateClientHandler(req: NextRequest, context: RouteContext) {
  const resolvedParams = await context.params;
  const targetDbId = resolvedParams.clientId;
...
async function patchClientHandler(req: NextRequest, context: RouteContext) {
  const resolvedParams = await context.params;
  const targetDbId = resolvedParams.clientId;
  const adminUserId = (req as any).user?.id;
  const ipAddress = req.ip || req.headers?.get('x-forwarded-for');
...
async function deleteClientHandler(req: NextRequest, context: RouteContext) {
  const resolvedParams = await context.params;
  const targetDbId = resolvedParams.clientId;
  const adminUserId = (req as any).user?.id!;
  const ipAddress = req.ip || req.headers?.get('x-forwarded-for');
