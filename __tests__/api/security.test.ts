...
function createAuthorizeRequest(authParams: Record<string, string>, method: 'GET' | 'POST' = 'GET'): NextRequest {
  if (method === 'GET') {
...
describe('Security Tests (SEC)', () => {
  ...
  describe('SEC-002: CSRF Protection Tests', () => {
    it('SEC-002: should protect against CSRF attacks on authorization endpoint', async () => {
      ...
      if (response.status === 403 || response.status === 400) {
        ...
        console.log(`⚠️ SEC-002: CSRF protection may not be implemented - status ${response.status}`)
      }
    ...
    it('should validate state parameter in OAuth flow', async () => {
      ...
      if (response.status === 302) {
        ...
        console.log('⚠️ OAuth authorize endpoint not available or state validation not implemented')
      }
  ...
  describe('SEC-004: Rate Limiting Tests', () => {
    it.skip('should implement rate limiting on token endpoint', async () => {
      ...
      console.log(`✅ Rate limiting test: ${successCount} success, ${rateLimitedCount} rate limited`)
