  ...
  describe('GET /api/clients/{clientId} - 获取客户端详情', () => {
    ...
    describe('✅ 正常流程测试', () => {
      ...
      it('应该返回客户端使用统计信息', async () => {
        // 创建一些使用记录
        await dataManager.createAccessToken(adminUser.id!, testClient.clientId, 'openid profile');

        const response = await httpClient.get(
          `/api/clients/${testClient.clientId}?includeStats=true`,
          {
            headers: {
              Authorization: `Bearer ${adminToken.token}`,
            },
    ...
    describe('✅ 正常流程测试', () => {
      ...
      it('应该支持重新生成客户端密钥', async () => {
        ...
        await httpClient.put(
          `/api/clients/${testClient.clientId}`,
          {
            clientType: 'confidential',
          },
          {
        ...
        const response = await httpClient.put(
          `/api/clients/${testClient.clientId}/regenerate-secret`,
          {},
          {
            headers: {
              Authorization: `Bearer ${adminToken.token}`,
    ...
    describe('❌ 异常处理测试', () => {
      ...
      it('应该拒绝删除有活跃用户的客户端（可选保护）', async () => {
        ...
        const response = await httpClient.delete(
          `/api/clients/${testClient.clientId}?force=false`,
          {
            headers: {
              Authorization: `Bearer ${adminToken.token}`,
            },
