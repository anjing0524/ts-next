...
describe('权限和作用域管理API测试', () => {
  ...
  describe('POST /api/scopes - 创建作用域', () => {
    ...
    describe('❌ 异常处理测试', () => {
      ...
      it('应该拒绝重复的作用域名称', async () => {
        // 先创建一个作用域
        await httpClient.post('/api/scopes', {
          name: 'duplicate:test',
          description: '重复测试作用域',
          category: 'test'
        }, {
  ...
  describe('PUT /api/scopes/{scopeName} - 更新作用域', () => {
    ...
    describe('❌ 异常处理测试', () => {
      it('应该处理不存在的作用域', async () => {
        const response = await httpClient.put('/api/scopes/non-existent', {
          description: '不存在的作用域'
        }, {
          headers: {
            'Authorization': `Bearer ${adminToken.token}`
      ...
      it('应该保护标准OpenID Connect作用域', async () => {
        ...
        const response = await httpClient.put(`/api/scopes/${standardScope.name}`, {
  ...
  describe('🔒 权限验证中间件测试', () => {
    ...
    it('应该支持作用域层级验证', async () => {
      ...
      // 只读令牌不应该能访问写入端点
      const writeResponse = await httpClient.post('/api/data', { test: 'data' }, {
        headers: {
  ...
  describe('🔄 作用域动态验证测试', () => {
    it('作用域更新后令牌权限应该实时生效', async () => {
      ...
      // 更新作用域，移除资源访问权限
      await httpClient.put(`/api/scopes/${dynamicScope.name}`, {
        resources: [] // 移除所有资源
      }, {
        headers: {
          'Authorization': `Bearer ${adminToken.token}`
