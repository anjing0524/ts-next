...
describe('OAuth2 UserInfo端点增强测试', () => {
  ...
  describe('正常流程测试', () => {
    ...
    it('应该根据profile作用域返回个人资料信息', async () => {
      const profileToken = await dataManager.createAccessToken(
        testUser.id,
        testClient.clientId,
        'openid profile'
      );
    ...
    it('应该根据email作用域返回邮箱信息', async () => {
      const emailToken = await dataManager.createAccessToken(
        testUser.id,
        testClient.clientId,
        'openid email'
      );
  ...
  describe('错误处理测试', () => {
    ...
    it('应该拒绝缺少openid作用域的令牌', async () => {
      const noOpenIdToken = await dataManager.createAccessToken(
        testUser.id,
        testClient.clientId,
        'profile email'
      );
    ...
    it('应该拒绝被撤销的访问令牌', async () => {
      const revokedToken = await dataManager.createAccessToken(
        testUser.id,
        testClient.clientId,
        'openid profile'
      );
  ...
  describe('作用域组合测试', () => {
    it('应该正确处理多个作用域组合', async () => {
      const multiScopeToken = await dataManager.createAccessToken(
        testUser.id,
        testClient.clientId,
        'openid profile email'
      );
  ...
  describe('边界条件测试', () => {
    it('应该处理不活跃的用户', async () => {
      ...
      const inactiveToken = await dataManager.createAccessToken(
        inactiveUser.id!,
        testClient.clientId,
        'openid profile'
      );
    ...
    it('应该处理邮箱未验证的用户', async () => {
      ...
      const unverifiedToken = await dataManager.createAccessToken(
        unverifiedUser.id!,
        testClient.clientId,
        'openid email'
      );
  ...
  describe('性能和并发测试', () => {
    it('应该能够处理并发请求', async () => {
      const concurrentRequests = Array(5)
        .fill(null)
        .map(() => httpClient.authenticatedRequest('/api/oauth/userinfo', accessToken));
