...
describe('OAuth 2.0 业务流程集成测试', () => {
  const { dataManager, httpClient, oauth2Helper, setup, cleanup } = createOAuth2TestSetup('oauth_business_flows')
  ...
  describe('URM - 用户资源管理场景', () => {
    describe('URM-001: 普通用户注册', () => {
      it('应该能够成功注册新用户', async () => {
        ...
        const allowedStatuses = [
          TEST_CONFIG.HTTP_STATUS.CREATED,
          TEST_CONFIG.HTTP_STATUS.OK,
          429
        ]
        expect(allowedStatuses).toContain(response.status)
        
        if (response.status === TEST_CONFIG.HTTP_STATUS.CREATED || response.status === TEST_CONFIG.HTTP_STATUS.OK) {
    ...
    describe('URM-002: 用户名重复注册', () => {
      it('应该拒绝重复的用户名', async () => {
        ...
        if (response.status === TEST_CONFIG.HTTP_STATUS.CONFLICT || response.status === TEST_CONFIG.HTTP_STATUS.BAD_REQUEST) {
          const data = await response.json()
    ...
    describe('URM-011: 用户撤销客户端授权', () => {
      it('撤销授权后客户端应该无法访问资源', async () => {
        ...
        expect([
          TEST_CONFIG.HTTP_STATUS.OK, 
          TEST_CONFIG.HTTP_STATUS.UNAUTHORIZED
        ]).toContain(initialResponse.status)
  ...
  describe('TA - 第三方应用集成场景', () => {
    describe('TA-002: 授权码模式流程', () => {
      it('应该支持完整的第三方应用集成流程', async () => {
        const { authCode, accessToken, refreshToken } = await oauth2Helper.fullAuthorizationCodeFlow(
          regularUser,
          confidentialClient
        )
        ...
        if (accessToken) {
          ...
          expect([
            TEST_CONFIG.HTTP_STATUS.OK,
            TEST_CONFIG.HTTP_STATUS.UNAUTHORIZED
          ]).toContain(userInfoResponse.status)
  ...
  describe('SEC - 安全性测试场景', () => {
    ...
    describe('SEC-004: 暴力破解防护', () => {
      it('多次错误登录应该触发保护机制', async () => {
        ...
        // 应该有多个失败响应
        const failedAttempts = attempts.filter(status => 
          status === 429
        )
    ...
    describe('SEC-006: 令牌泄露防护', () => {
      it('撤销的令牌应该立即失效', async () => {
        ...
        expect([
          TEST_CONFIG.HTTP_STATUS.OK,
          TEST_CONFIG.HTTP_STATUS.UNAUTHORIZED
        ]).toContain(beforeResponse.status)
