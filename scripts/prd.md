...
- **精细化权限管理**：管理用户可访问的菜单（页面）和API接口权限
- **多场景支持**：支持认证中心认证、服务间认证、API Server服务认证、第三方客户端认证等典型场景
- **高性能要求**：内网环境下追求更高的认证和权限验证性能
...
### 2.6 ABAC（基于属性的访问控制）策略引擎

#### 2.6.1 核心概念与策略模型

ABAC通过评估描述访问请求的属性来动态决定访问权限。这些属性通常分为四类：

- **主体 (Subject)**：发起访问请求的实体，通常是用户。主体的属性可能包括：

  - 用户ID、用户名
  - 所属角色 (e.g., `user.roles = ['manager', 'editor']`)
  - 所属部门 (e.g., `user.department = 'Sales'`)
  - 安全级别/许可 (e.g., `user.securityClearance = 'Level2'`)
  - 地理位置 (e.g., `user.location = 'OfficeA'`)
  - 其他自定义属性 (e.g., `user.projectTeam = 'Alpha'`)

- **资源 (Resource)**：被访问的目标对象或数据。资源的属性可能包括：

  - 资源ID、资源名称
  - 资源类型 (e.g., `resource.type = 'document'` or `resource.type = 'api_endpoint'`)
  - 资源所有者 (e.g., `resource.owner = 'user123'`)
  - 敏感级别 (e.g., `resource.sensitivity = 'Confidential'`)
  - 创建时间、修改时间
  - 所属项目/分类 (e.g., `resource.project = 'ProjectX'`, `resource.category = 'FinancialData'`)
  - 地理标签 (e.g., `resource.dataResidency = 'EU'`)

- **操作 (Action)**：主体尝试对资源执行的具体动作。操作的属性通常是操作的类型，例如：
...
- **环境 (Environment/Context)**：访问请求发生时的上下文条件。环境属性可能包括：

  - 时间 (e.g., `environment.time.hourOfDay`, `environment.time.dayOfWeek`)
  - IP地址 (e.g., `environment.request.ipAddress`)
  - 设备类型 (e.g., `environment.device.type = 'mobile'`)
  - 网络位置 (e.g., `environment.network.location = 'internal'`)
  - 请求协议 (e.g., `environment.request.protocol = 'https'`)
  - 当前系统负载、威胁级别等动态因素。

- **策略 (Policy)**：一组或多组规则，这些规则基于主体、资源、操作和环境的属性来定义访问是否被允许 (Permit) 或拒绝 (Deny)。
  - **规则 (Rule)**：一个规则通常包含一个条件表达式和一个效果 (Permit/Deny)。
  - **条件 (Condition)**：一个逻辑表达式，它评估主体、资源、操作和环境的属性。例如：`user.department == resource.department AND action.type == 'read'`。
  - **策略组合算法 (Policy Combining Algorithm)**：当多个策略或规则适用于同一个访问请求时，需要一个明确的算法来决定最终的访问决策（例如：Deny-Overrides, Permit-Overrides, First-Applicable）。

#### 2.6.2 策略管理功能

**UC-2.6.2-001: 策略创建与管理**

- **前置条件**：操作者拥有“策略管理”权限（通常是高级管理员或安全管理员）。
- **主要流程**：
  1.  **创建新策略**：管理员可以创建新的访问策略，定义策略的名称（需唯一）、描述、版本和生效范围（例如，全局策略、特定应用策略）。
  2.  **定义策略规则**：
      - 选择或定义策略适用的主体属性条件 (e.g., `user.role IN ['manager']`, `user.department == 'HR'`)。
      - 选择或定义策略适用的资源属性条件 (e.g., `resource.type == 'financial_report'`, `resource.sensitivity == 'high'`)。
      - 选择或定义策略适用的操作类型 (e.g., `action.type IN ['read', 'update']`)。
      - 选择或定义策略适用的环境属性条件 (e.g., `environment.time.hourOfDay >= 9 AND environment.time.hourOfDay <= 17`, `environment.request.ipAddress IN office_ip_range`)。
      - 指定规则的效果：允许 (Permit) 或拒绝 (Deny)。
  3.  **策略编辑器**：提供用户友好的界面（可能是可视化编辑器或结构化文本/DSL编辑器）来构建和编辑策略规则和条件。
  4.  **设置策略优先级与冲突解决**：定义当多个策略规则都匹配一个访问请求时，如何决定最终结果（例如，通过策略的优先级排序，或使用预定义的策略组合算法如 Deny-Overrides）。
  5.  **策略版本控制与生命周期管理**：支持策略的版本管理，允许查看历史版本、回滚到旧版本。策略应有启用/禁用状态。
  6.  **策略测试与模拟** (可选，但强烈推荐)：提供工具让管理员可以模拟访问请求（输入主体、资源、操作、环境属性），测试策略的评估结果，以验证策略的正确性。
  7.  **保存与激活策略**：保存策略定义，并将其激活以在权限验证流程中生效。所有策略变更应记录审计日志。
- **验收标准**：
  - 策略规则的逻辑表达清晰、准确，无歧义，能够正确反映预期的访问控制意图。
  - 策略编辑器易于使用，能够支持复杂的条件组合。
  - 策略的优先级和冲突解决机制（策略组合算法）按预期工作。
  - 策略的启用、禁用和版本变更能够实时或在合理延迟内生效。
  - 策略管理操作（创建、修改、删除、启用/禁用）均有详细的审计日志。
  - （若实现）策略测试功能能够准确模拟策略评估结果。

#### 2.6.3 策略评估引擎 (内部机制，非直接用户用例)

- **机制描述**：当系统收到一个访问请求时，权限验证组件会将请求中的主体、资源、操作和环境属性提交给ABAC策略评估引擎。引擎会匹配所有适用的策略规则，并根据定义的策略组合算法计算出最终的访问决策（Permit/Deny）。
- **关键组件**：
  - **属性收集器 (PIP - Policy Information Point)**：负责从各种来源（如用户目录、资源数据库、请求上下文）收集和提供评估所需的属性信息。
  - **策略决策点 (PDP - Policy Decision Point)**：核心组件，根据已定义的策略和收集到的属性，评估并做出访问决策。
  - **策略执行点 (PEP - Policy Enforcement Point)**：负责拦截访问请求，将请求信息发送给PDP获取决策，并强制执行该决策（允许或拒绝访问）。

#### 2.6.4 策略示例（用于说明和测试）

以下是一些ABAC策略的示例，用于阐明其应用场景：

1.  **部门数据隔离策略**：

    - **描述**：用户只能读取其所在部门创建的文档。
    - **主体条件**：`user.department`
    - **资源条件**：`resource.type == 'document' AND resource.creatingDepartment == user.department`
    - **操作条件**：`action.type == 'read'`
    - **环境条件**：无特定环境条件
    - **效果**：Permit
    - **规则**：`IF (user.department == resource.creatingDepartment AND resource.type == 'document' AND action.type == 'read') THEN Permit`

2.  **工作时间与地点访问敏感系统策略**：

    - **描述**：只有在工作日的工作时间（周一至周五，9:00-18:00）且从办公网IP段访问时，特定角色的用户（如“财务审计员”）才能访问“财务审计系统”。
    - **主体条件**：`user.roles CONTAINS 'financial_auditor'`
    - **资源条件**：`resource.name == 'FinancialAuditSystem'`
    - **操作条件**：`action.type == 'access'` (或更具体的操作)
    - **环境条件**：`environment.time.dayOfWeek IN ['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY'] AND environment.time.hourOfDay >= 9 AND environment.time.hourOfDay < 18 AND environment.request.ipAddress IN office_ip_range`
    - **效果**：Permit

3.  **基于风险的访问控制策略（动态）**：

    - **描述**：如果用户尝试从一个异常地理位置或在一个高风险时段（例如，系统维护窗口之外的深夜）执行敏感操作（如“删除关键数据”），则拒绝访问，除非该用户通过了额外的MFA验证。
    - **主体条件**：`user.riskScore > 'medium' OR environment.request.isAnomalousLocation == true`
    - **资源条件**：`resource.sensitivity == 'critical'`
    - **操作条件**：`action.type == 'delete'`
    - **环境条件**：(由风险引擎动态评估，或 `environment.time.isHighRiskPeriod == true`)
    - **效果**：Deny (除非 `user.mfaVerifiedRecently == true`，这可以作为另一个Permit规则或此Deny规则的例外条件)

4.  **特定角色对特定类型资源的操作限制**：
    - **描述**：“项目经理”角色可以对其管理的项目（`user.managedProjects CONTAINS resource.projectID`）中的“任务”资源（`resource.type == 'task'`）执行“创建”、“更新”、“分配”操作，但不能“删除”已完成的任务（`resource.status == 'completed'`）。
    - **规则1 (Permit CUD)**:
      - **主体**：`user.roles CONTAINS 'project_manager' AND user.managedProjects CONTAINS resource.projectID`
      - **资源**：`resource.type == 'task'`
      - **操作**：`action.type IN ['create', 'update', 'assign']`
      - **效果**：Permit
    - **规则2 (Deny Delete Completed Task)**:
      - **主体**：`user.roles CONTAINS 'project_manager'` (或更宽松，因为Deny优先)
      - **资源**：`resource.type == 'task' AND resource.status == 'completed'`
      - **操作**：`action.type == 'delete'`
      - **效果**：Deny
      - (需要Deny-Overrides的策略组合算法)
...
- **核心逻辑**：
  ...
  2.  首先执行RBAC检查：根据用户角色（`subjectAttributes.roles` 或通过 `userId` 查询）判断是否拥有对 `resourceAttributes.resourceId` 执行 `action.type` 的直接权限或继承权限。
  3.  如果RBAC允许，则进一步执行ABAC策略评估：使用所有属性（主体、资源、操作、环境）匹配已定义的ABAC策略。
  4.  根据RBAC结果和ABAC策略评估结果（以及策略组合算法），做出最终的 `allowed` 决策。
  5.  （可选）如果RBAC直接拒绝且没有覆盖性的ABAC Permit策略，则直接返回拒绝。
- **验收标准**：
  - 权限验证逻辑正确结合RBAC和ABAC规则：
    - 如果RBAC检查通过，且没有ABAC策略明确拒绝，则允许。
    - 如果RBAC检查不通过，但有ABAC策略明确允许，则允许（取决于ABAC是否可以覆盖RBAC）。
    - 如果RBAC检查通过，但有ABAC策略明确拒绝，则拒绝（Deny-Overrides）。
    - 需要明确RBAC和ABAC的决策组合逻辑。
...
#### 2.8.4 ABAC策略管理 (Policy Management) (若实现ABAC)

**UC-2.8.4-001: ABAC策略列表查看与筛选** (关联UC-2.6.2-001)

- **功能描述**：管理员可以查看所有已定义的ABAC策略列表。
- **主要流程**：
  1.  访问ABAC策略管理页面。
  2.  列表显示策略名称、描述、版本、状态（启用/禁用）、优先级等。
  3.  支持按策略名称或状态筛选。
- **验收标准**：
  - 策略列表显示准确。

**UC-2.8.4-002: ABAC策略创建与编辑** (关联UC-2.6.2-001)

- **功能描述**：管理员可以使用可视化或DSL编辑器创建和编辑ABAC策略规则。
- **主要流程**：
  1.  点击“创建策略”或选择已有策略编辑。
  2.  定义策略名称、描述。
  3.  使用编辑器构建策略规则：选择主体属性、资源属性、操作、环境属性，并定义其逻辑关系和条件值。
  4.  设置策略效果（Permit/Deny）和优先级。
  5.  （可选）进行策略模拟测试。
  6.  保存并激活/禁用策略。
- **验收标准**：
  - 策略编辑器用户友好，支持构建复杂的逻辑条件。
  - 策略规则能被正确保存和解析。
  - 策略优先级和状态能被正确设置和应用。
  - （若有）策略模拟测试功能准确。
  - 所有操作记录审计日志。
...
- **目标**: 实现ABAC策略引擎（如果采纳），完善权限验证机制，丰富管理界面功能，加强系统安全性和可观测性。
- **主要任务**:
  - [ ] **ABAC策略引擎模块 (UC-2.6) (Week 7-8)** (如果采纳): 实现策略定义、存储、评估引擎。
  - [ ] **统一权限验证模块 (UC-2.7) (Week 8-9)**: 实现单项和批量API权限验证接口，开发客户端SDK雏形。
  - [ ] **管理界面 - 角色权限与ABAC策略管理 (UC-2.8.2, UC-2.8.4) (Week 9-10)**: 实现角色权限分配界面，ABAC策略管理界面。
...
| :--------------- | :---------------------------------------------------------------------------------- | :----- | :------- | :------------------------------------------------------------------------------------------------------------------------------------ | :--------- |
| **技术风险**     | ABAC策略引擎设计与实现复杂度高，可能超出预期。                                      | 中     | 高       | - 初期进行充分的技术预研和原型验证。<br>- 采用成熟的规则引擎库或逐步实现核心功能。<br>- 投入有经验的开发人员。                        | 技术负责人 |
|                  | 对选型技术栈（如NestJS, Prisma, oidc-provider）的掌握程度不足，影响开发效率和质量。 | 中     | 中       | - 组织内部培训和知识分享。<br>- 鼓励团队成员学习官方文档和最佳实践。<br>- 在非核心模块先行试点。                                      | 技术负责人 |
