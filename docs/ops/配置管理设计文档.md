...
# pingora-proxy 配置管理设计（2025-07-08）

## 配置文件位置
- 路径：`apps/pingora-proxy/config/default.yaml`
- 格式：YAML

## 配置项说明
```yaml
listen_addr: "0.0.0.0:6180"           # 代理服务监听地址
prometheus_addr: "0.0.0.0:6192"       # Prometheus 监控监听地址（可选）
jwt:
  jwks_url: "https://example.com/.well-known/jwks.json"  # JWKS 公钥地址
  audience: "your-audience"            # JWT 受众
  issuer: "your-issuer"                # JWT 签发方
upstreams:
  - name: "service1"
    backend: "http://127.0.0.1:8001"   # 上游服务地址
    health_path: "/health"             # 健康检查路径（可选）
  - name: "service2"
...
## 环境变量覆盖机制
- 支持通过环境变量覆盖配置，前缀为 `PINGORA__`，下划线用双下划线分隔。
- 例如：
  - `PINGORA__LISTEN_ADDR=0.0.0.0:7000` 覆盖监听端口
  - `PINGORA__JWT__JWKS_URL=https://xxx` 覆盖 JWKS 地址
...
## 代码实现
- 详见 `src/config/mod.rs`，使用 config + serde_yaml 自动加载
- main.rs 启动时统一加载配置

## 变更记录
- 2025-07-08：首次补充配置项说明，支持环境变量覆盖
