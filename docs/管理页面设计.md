...
- ✅ **微服务管理中心**: 作为 Turborepo Monorepo 微服务架构的统一管理入口
- ✅ **OAuth2.1 客户端**: 作为 `oauth-service` 的标准 OAuth2.1 客户端，完全遵循授权流程
- ✅ **现代化技术栈**: 基于 Next.js 15.3.2 + React 19 + shadcn/ui + Tailwind CSS 4.1.5
- ✅ **企业级 RBAC**: 支持四级权限控制（菜单、API、数据、操作）
- ✅ **响应式设计**: 支持桌面端和移动端访问

**技术特色**:

- 🎨 **shadcn/ui 组件库**: 基于 Radix UI 的现代化组件系统
- 🔐 **强制 PKCE**: 所有 OAuth2.1 流程强制使用 PKCE (S256)
- 📊 **数据表格**: @tanstack/react-table 高性能数据展示
- 🎯 **类型安全**: 100% TypeScript 5 严格模式
- 🚀 **性能优化**: Next.js 15 App Router + React 19 并发特性

## 概述

本文档详细描述了 `admin-portal` 应用（端口: 3002）的页面功能和实现完整性。`admin-portal` 是整个 OAuth2.1 认证授权中心和微服务平台的核心管理界面，负责用户管理、角色权限分配、OAuth客户端管理、系统监控等功能。它严格遵循 OAuth2.1 + 强制 PKCE 标准与 `oauth-service` 进行安全通信，为管理员提供强大而安全的操作体验。
