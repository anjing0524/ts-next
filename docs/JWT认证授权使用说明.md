...
**文档摘要**: 本文档详细说明了基于 Turborepo 的 OAuth2.1 认证授权中心和微服务平台中如何获取、验证和使用JWT令牌。本系统严格遵循OAuth2.1标准，**绝对没有独立的login端点**，所有认证操作必须通过标准的OAuth2.1授权码流程完成。
...
- ❌ **完全不支持直接用户名/密码认证API**
- ❌ **绝不使用jsonwebtoken库**
- ✅ **100%只支持OAuth2.1授权码流程 + PKCE**
- ✅ **强制使用PKCE安全机制 (S256)**
- ✅ **100%使用jose库 (v6.0.11) 处理JWT**
- ✅ **固定使用RS256签名算法**
- ✅ **支持完整的OIDC (OpenID Connect)**

**技术栈确认**:

```json
{
  "架构": "Turborepo Monorepo + 5个微服务",
  "JWT处理库": "jose@6.0.11",
  "签名算法": "RS256 (RSA-SHA256)",
  "认证流程": "OAuth2.1 + 强制PKCE + OIDC",
  "令牌格式": "JWT (自包含 + 数字签名)",
  "框架": "Next.js 15.3.2 + React 19",
  "数据库": "Prisma 6.10.1 + SQLite/PostgreSQL",
  "密钥管理": "JWKS (JSON Web Key Set)",
  "jsonwebtoken": "绝对不使用"
}
```
...
// 1. 导入RSA私钥
const privateKey = await jose.importPKCS8(process.env.JWT_PRIVATE_KEY_PEM, 'RS256');
...
// 使用JWKS端点验证 (推荐方式)
const JWKS = jose.createRemoteJWKSet(new URL('https://auth.company.com/.well-known/jwks.json'));
...
const publicKey = await jose.importSPKI(process.env.JWT_PUBLIC_KEY_PEM, 'RS256');
