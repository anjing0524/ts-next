...
```mermaid
graph TB
    subgraph "OAuth2.1 认证授权中心 & 微服务平台"
        subgraph "Apps Layer - 微服务应用层"
            A1["🔐 oauth-service<br/>OAuth2.1 认证授权服务<br/>Port: 3001"]
            A2["🎛️ admin-portal<br/>系统管理后台<br/>Port: 3002"]
            A3["📈 kline-service<br/>K线图表服务<br/>Port: 3003"]
            A4["🔄 flow-service<br/>可视化流程引擎<br/>Port: 3004"]
            A5["🧪 test-service<br/>测试与示例服务<br/>Port: 3005"]
        end

        subgraph "Packages Layer - 共享包层"
            P1["@repo/ui<br/>UI组件库<br/>shadcn/ui + Tailwind"]
            P2["@repo/lib<br/>工具函数库<br/>认证、验证、工具"]
            P3["@repo/database<br/>数据库层<br/>Prisma + SQLite"]
            P4["@repo/eslint-config<br/>代码规范配置"]
            P5["@repo/typescript-config<br/>TypeScript配置"]
        end
    end

    subgraph "External Systems - 外部系统"
        DB[("🗄️ SQLite Database<br/>用户、角色、权限<br/>OAuth客户端")]
        CLIENT["📱 OAuth2.1 Clients<br/>第三方应用<br/>移动端/Web端"]
        BROWSER["🌐 Web Browser<br/>管理员界面<br/>用户授权界面"]
    end
