...
- **pingora-proxy**：API网关（端口 6180）
- **oauth-service**：认证服务（端口 3001），仅提供API
- **admin-portal**：管理后台+认证中心UI（端口 3002）
- **packages/**：共享数据库/UI组件/工具库

### 3.2 服务分工

- **oauth-service**：
  - 提供OAuth2.1/OIDC/RBAC的API接口
  - 需用户交互时重定向到admin-portal对应页面

- **admin-portal**：
  - 负责所有认证页面（登录/授权/错误处理）
  - 根据oauth-service参数渲染页面
  - 收集用户输入后回调oauth-service API

### 3.3 核心流程

1. 未认证请求 → oauth-service/authorize 
2. 重定向 → admin-portal/auth/login
3. 登录成功 → oauth-service继续流程
4. 需授权 → admin-portal/oauth/consent
5. 用户决策 → oauth-service颁发令牌
