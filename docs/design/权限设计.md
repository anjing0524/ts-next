...
## 1.1 OAuth2 和 RBAC 集成最佳实践

基于最新最佳实践，OAuth2 可用于令牌认证，而 RBAC 通过角色控制访问。推荐使用 Quarkus 或 WSO2 Identity Server 实现 scope-based 访问控制，将角色映射到 OAuth2 scopes 以实现细粒度授权。<mcreference link="https://quarkus.io/guides/security-oauth2" index="1">1</mcreference> <mcreference link="https://gdrdabarera.medium.com/rbac-role-based-access-control-via-oauth2-0-scopes-with-wso2-identity-server-dd6dcb0b737b" index="2">2</mcreference> <mcreference link="https://auth0.com/docs/manage-users/access-control/rbac" index="5">5</mcreference>

- 使用 JWT 令牌携带角色声明。
- 在微服务中，通过 OIDC 验证令牌并应用 RBAC。<mcreference link="https://stackoverflow.com/questions/9604191/oauth2-and-role-based-access-control" index="3">3</mcreference> <mcreference link="https://security.stackexchange.com/questions/206491/microservices-and-rbac-using-oidc" index="4">4</mcreference>
...
在OAuth2授权码流程中，登录页面(`/auth/login`)是用户身份认证的关键环节：

1. 当未认证用户访问受保护资源时，重定向到登录页面
2. 用户输入用户名密码进行身份验证
3. 验证成功后继续OAuth授权流程
4. 验证失败则返回错误信息
...
| 权限点                 | 描述             | 类型  |
| :--------------------- | :--------------- | :---- |
| `auth-center:interact` | 与认证中心UI交互 | `API` |
...
- **统一权限校验**: 通过 apps/oauth-service/middleware.ts 实现后端敏感API路由权限校验
- **自动校验**: 所有 POST/PUT/DELETE 敏感操作自动校验权限点
- **权限映射**: 权限点与 permission-map.ts 保持一致
- **错误处理**: 权限校验失败返回 403，前端统一处理
- **审计记录**: 权限检查操作自动记录到审计日志

### 架构分工确认

- **oauth-service**: 仅提供API，不渲染页面，负责权限校验
- **admin-portal**: 负责所有认证页面（登录、授权、错误处理）
- **权限同步**: JWT中携带用户权限信息，实现前后端权限一致
