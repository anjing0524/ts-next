...
#### SQLite Configuration (Primary Production)
 
The system uses SQLite as the primary database for production, leveraging Kubernetes Persistent Volumes (PVC) for data durability.
...
# Verify database in K8s pod
kubectl exec -it -n oauth-system deploy/oauth-service -- ls -la /app/data/
 
# Verify database integrity
kubectl exec -it -n oauth-system deploy/oauth-service -- sqlite3 /app/data/oauth.db ".tables"
```
 
##### SQLite Backup Strategy
 
```bash
# Backup from K8s Volume
kubectl exec -n oauth-system deploy/oauth-service -- sqlite3 /app/data/oauth.db ".backup '/tmp/backup.db'"
kubectl cp oauth-system/$(kubectl get pod -n oauth-system -l app=oauth-service -o jsonpath='{.items[0].metadata.name}'):/tmp/backup.db ./oauth-backup-$(date +%Y%m%d).db
```
 
### Alternative / Future Database Configuration
 
#### MySQL Configuration (Optional)
 
> **Note**: MySQL is currently an alternative configuration and not the default for this release.
