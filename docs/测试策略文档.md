...
本文档定义了OAuth2.1认证授权中心的TDD（测试驱动开发）策略，采用真实数据测试而非过度mock，重点关注OAuth2.1标准流程和RBAC权限模型的正确实现。

**TDD核心原则**: 
- ✅ **Red-Green-Refactor循环**：先写失败测试，再写最小代码，最后重构
- ✅ **测试行为而非实现**：关注业务逻辑和用户行为验证
- ✅ **真实数据测试**：使用真实数据库和测试用户，避免过度mock
- ✅ **小步迭代**：每次只测试一个小功能，快速反馈
- ✅ **独立性原则**：每个测试独立运行，不依赖其他测试

**重要更新**: 
- ✅ 统一使用Jest作为测试框架，移除Vitest
- ✅ **删除所有性能测试相关内容**
- ✅ 修复Jose库ESM导入问题
- ✅ 建立真实测试用户和权限数据
- ✅ 专注于OAuth2.1授权码流程和RBAC测试
