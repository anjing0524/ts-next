...
import { BatchPermissionCheckRequestSchema, BatchPermissionCheckRequestType, BatchIndividualCheckRequestType } from './schemas'; // Path relative to current dir
...
async function checkBatchPermissionHandler(request: NextRequest, context: AuthContext) {
  ...
  const serviceRequests: PermissionServiceRequest[] = clientRequests.map((req: BatchIndividualCheckRequestType) => ({
  ...
  const responseResults = serviceResults.map(sr => ({
    ...
    message: sr.message || (sr.allowed ? 'Operation allowed for this item.' : 'Operation denied for this item.'),
    // matchedPolicyIds: sr.matchedPolicyIds || [] // Optional, if your service provides this
  }));

  return NextResponse.json(successResponse({ results: responseResults }, 200, 'Batch permission check completed.', requestId), { status: 200 });
}

export const POST = withErrorHandler(withAuth(checkBatchPermissionHandler, {
