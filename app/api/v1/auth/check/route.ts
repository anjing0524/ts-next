...
async function checkSinglePermissionHandler(request: NextRequest, context: AuthContext) {
  ...
  const hasPermission = await permissionService.checkPermission(subjectAttributes.userId, permissionName);
  ...
  return NextResponse.json(successResponse(decision, 200, 'Permission check completed.', requestId), { status: 200 });
}

export const POST = withErrorHandler(withAuth(checkSinglePermissionHandler, {
