...
import { OAuth2Error, OAuth2ErrorCode, ValidationError, ResourceNotFoundError, ConfigurationError, BaseError } from '@/lib/errors';
...
async function getConsentPageDataHandlerInternal(request: AuthenticatedRequest): Promise<NextResponse> {
  const authUser = request.user;
  if (!authUser || !authUser.id) {
    // 理论上已被 requirePermission 捕获，但作为深度防御
    // Theoretically caught by requirePermission, but as defense-in-depth
    throw new BaseError('User context not found after permission check.', 500, 'SERVER_SETUP_ERROR_CONSENT_GET');
  }
  ...
  if (!user) {
    ...
    throw new ResourceNotFoundError('Authenticated user not found or inactive. Please re-authenticate.', 'AUTH_USER_INVALID_CONSENT_GET', { userId });
  }
  ...
  if (!clientId || !redirectUri || !scopeString || !responseType) {
    ...
    throw new ValidationError('Missing required parameters from authorization server.', OAuth2ErrorCode.InvalidRequest, {
        missing: [!clientId && 'client_id', !redirectUri && 'redirect_uri', !scopeString && 'scope', !responseType && 'response_type'].filter(Boolean)
    });
  }
  if (responseType !== 'code') {
    // 无效的 response_type
    // Invalid response_type
    throw new ValidationError('Invalid response_type from authorization server: must be "code".', OAuth2ErrorCode.UnsupportedResponseType, { responseType });
  }
  ...
  if (!client || !client.isActive) {
    ...
    throw new OAuth2Error('Third-party client not found or is not active.', OAuth2ErrorCode.InvalidClient, 403, undefined, { clientId });
  }
  ...
  try {
    ...
    throw new ConfigurationError('Server error: Invalid client configuration for redirectUris.', 'CLIENT_CONFIG_REDIRECT_URI_INVALID_CONSENT_GET', { clientId });
  }
  if (!AuthorizationUtils.validateRedirectUri(redirectUri, registeredRedirectUris)) {
      ...
      throw new ValidationError('Provided redirect_uri is not registered for this client.', OAuth2ErrorCode.InvalidRequest, { redirectUri, registered: registeredRedirectUris });
  }
  ...
  if (requestedScopesDetails.length !== requestedScopeNames.length) {
    ...
    throw new ValidationError(`The following requested permissions are invalid or inactive: ${missing.join(', ')}. Please contact the application developer.`, OAuth2ErrorCode.InvalidScope, { missingScopes: missing });
  }

  const responseData: ConsentPageData = {
    client: { id: client.clientId, name: client.clientName || client.clientId, logoUri: client.logoUri },
    requested_scopes: requestedScopesDetails,
    user: { id: user.id, username: user.username },
    consent_form_action_url: CONSENT_API_URL_PATH,
    client_id: clientId, redirect_uri: redirectUri, scope: scopeString,
    state: state || undefined, response_type: responseType,
...
async function submitConsentDecisionHandlerInternal(request: AuthenticatedRequest): Promise<NextResponse> {
  const authUser = request.user;
   if (!authUser || !authUser.id) {
    throw new BaseError('User context not found after permission check.', 500, 'SERVER_SETUP_ERROR_CONSENT_POST');
  }
  const userId = authUser.id;

  const user = await prisma.user.findUnique({ where: { id: userId, isActive: true }});
  if (!user) {
    throw new ResourceNotFoundError('Authenticated user not found or inactive.', 'AUTH_USER_INVALID_CONSENT_POST', { userId });
  }
  ...
  if (contentType?.includes('application/x-www-form-urlencoded')) {
    ...
    throw new ValidationError('Unsupported Content-Type. Please use application/x-www-form-urlencoded or application/json.', 'UNSUPPORTED_MEDIA_TYPE_CONSENT_POST', 415);
  }
  ...
  if (!decision || !clientId || !grantedScopeString || !redirectUri || !responseType) {
    ...
    throw new ValidationError('Missing required form fields from consent submission.', OAuth2ErrorCode.InvalidRequest, {
        missing: [!decision && 'decision', !clientId && 'client_id', !grantedScopeString && 'scope', !redirectUri && 'redirect_uri', !responseType && 'response_type'].filter(Boolean)
    });
  }
  if (responseType !== 'code') {
    throw new ValidationError('Invalid response_type submitted from consent form: must be "code".', OAuth2ErrorCode.UnsupportedResponseType, { responseType });
  }
  ...
  if (!client || !client.isActive) {
    ...
    throw new OAuth2Error('Client specified in consent form not found or is not active.', OAuth2ErrorCode.InvalidClient, 403, undefined, { clientId });
  }
  let registeredRedirectUris: string[] = [];
  ...
  if (!AuthorizationUtils.validateRedirectUri(redirectUri, registeredRedirectUris)) {
      ...
      throw new ValidationError('Invalid redirect_uri submitted from consent form for this client.', OAuth2ErrorCode.InvalidRequest, { redirectUri });
  }
  ...
  throw new ValidationError('Invalid decision value submitted. Must be "allow" or "deny".', OAuth2ErrorCode.InvalidRequest, { decision });
}

// 使用 withErrorHandling 和 requirePermission 包装处理函数
// Wrap handlers with withErrorHandling and requirePermission
export const GET = withErrorHandling(requirePermission('auth-center:interact')(getConsentPageDataHandlerInternal));
export const POST = withErrorHandling(requirePermission('auth-center:interact')(submitConsentDecisionHandlerInternal));
