...
export const scopeResponseSchema = z.object({
  id: z.string().cuid(),
  name: z.string().describe("作用域的唯一编码名称 (Unique coded name for the scope, e.g., openid, profile, user:read)"),
  // displayName: z.string().optional().nullable().describe("作用域的显示名称 (Display name for the scope)"), // Not in Prisma Scope model
  description: z.string().nullable().optional().describe("作用域的详细描述 (Detailed description of the scope)"),
  isPublic: z.boolean().describe("此Scope是否对所有公开客户端可用 (Is this scope available to all public clients)"),
...
export const scopeListQuerySchema = z.object({
  ...
  sortBy: z.string().default('name').optional().describe("排序字段 (Sort by field, e.g., name, createdAt)"),
...
export const scopeCreateSchema = z.object({
  name: z.string().min(1, "作用域名称不能为空").max(100, "作用域名称长度不能超过100个字符")
    .regex(/^[a-zA-Z0-9_:-]+$/, "作用域名称格式无效 (Invalid scope name format. Allowed: letters, numbers, underscore, colon, hyphen)"),
