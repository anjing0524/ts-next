...
const CreatePermissionSchema = z
  .object({
    ...
    name: z
      ...
      .regex(
        /^[a-z0-9_.:-]+$/,
        'Name can only contain lowercase letters, numbers, underscores, colons, and hyphens'
      ),
...
async function createPermission(request: NextRequest, authContext: AuthContext) {
  try {
    ...
    await AuthorizationUtils.logAuditEvent({
      ...
      success: false,
      errorMessage:
        error instanceof Error ? error.message : 'Unknown error while creating permission',
      ipAddress: request.ip || request.headers.get('x-forwarded-for'),
      userAgent: request.headers.get('user-agent'),
    });
...
async function listPermissions(request: NextRequest, authContext: AuthContext) {
  try {
    ...
    if (displayNameQuery)
      whereClause.displayName = { contains: displayNameQuery, mode: 'insensitive' };
...
export const POST = withAuth(createPermission, {
